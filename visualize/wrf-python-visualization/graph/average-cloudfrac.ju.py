...
all_mean_values, all_mean_values_urb, all_mean_values_n_urb = [], [], []
cut_mean_values, cut_mean_values_urb, cut_mean_values_n_urb = [], [], []
...
#     ahe = 100
    ...
    all_t2_values = getvar(dataset, "CLDFRA", timeidx=ALL_TIMES)
    all_t2_values_rem_urb = np.copy(all_t2_values)
    all_t2_values_rem_urb[:, :, 40:60, 40:60] = 0

    cut_t2_value, cut_t2_value_rem_urb = [], []
    for time_idx, date_time in enumerate(all_times):
        ...
        cut_t2_value_rem_urb.append(all_t2_values_rem_urb[time_idx])

    cut_t2_value = np.array(cut_t2_value)
    cut_t2_mean = np.mean(cut_t2_value)
    cut_t2_mean_urb = np.mean(cut_t2_value[:, :, 40:60, 40:60])

    cut_mean_values.append(cut_t2_mean)
    cut_mean_values_urb.append(cut_t2_mean_urb)
    cut_mean_values_n_urb.append(np.mean(np.array(cut_t2_value_rem_urb)))

    all_mean_values.append(np.mean(all_t2_values))
    all_mean_values_urb.append(np.mean(all_t2_values[:, :, 40:60, 40:60]))
    all_mean_values_n_urb.append(np.mean(all_t2_values_rem_urb))


axes[0].set_title('Mean Cloud Fraction Across All Time vs. AHE Value')
...
axes[1].set_title('Mean Cloud Fraction Across All\n Time in Urban Area vs. AHE Value')
...
axes[2].set_title('Mean Cloud Fraction Across All\n Time in Non-Urban Area vs. AHE Value')
axes[2].plot(iterate_values, all_mean_values_n_urb, '^--', linewidth=1, color="red", label="Default")
axes[2].plot(iterate_values, cut_mean_values_n_urb, 'o-', linewidth=1, color="black", label="No spin up")

for i in range(3):
    axes[i].set_xlabel('AHE (W.m-2)')
    # axes[i].set_xlabel('Z0 (m)')
    axes[i].set_ylabel('Mean Cloud Fraction')
    axes[i].legend()
    axes[i].grid()

plt.savefig('/home/guc/results/z0-1-ahe-mean-cloud-frac.png')
