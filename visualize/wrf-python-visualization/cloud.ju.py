...
is_py = os.path.basename(sys.argv[0]) == 'cloud.ju.py'
...
def plot_image(dataset: Dataset, debug: bool = False, has_latlon: bool = True) -> None:
    ...
    for time_idx, datetime in enumerate(times):
        ...
        var_data = getvar(dataset, "CLDFRA", time_idx)
        ...
        if has_latlon:
            ...
            orig_lats, orig_lons = latlon_coords(z_data)
            latlon_string = f' ({to_np(orig_lats)[0][0]:0f},{to_np(orig_lons)[0][0]:0f})'
            lons = np.arange(0, var_data.shape[1])
            lats = np.arange(0, var_data.shape[2])
            plt.figure(figsize=(7, 6)) # WIDTH, HEIGHT

        var_np_data = np.mean(to_np(var_data), axis=0)
        plt.contourf(lons, lats, var_np_data, plot_lim, cmap="Spectral_r",
        ...
        plt.title(f"Cloud Fraction, Real Model,\nNo Coriolis Urban Grassland ideal condition at\n{datetime_string}{latlon_string}")
